...
async function initDB() {
  ...
  // 经手人费用明细（杨德彪等）
  db.run(`CREATE TABLE IF NOT EXISTS agent_expenses (
    id INTEGER PRIMARY KEY, date TEXT, base TEXT, person TEXT, item TEXT,
    quantity REAL DEFAULT 0, unit_price REAL DEFAULT 0,
    amount REAL DEFAULT 0, subtotal REAL DEFAULT 0, notes TEXT DEFAULT ''
  )`);
  // 临时工劳务清单
...
const TABLE_MAP = {
  ...
  agentExpenses: { table: 'agent_expenses', fields: ['date','base','person','item','quantity','unit_price','amount','subtotal','notes'] },
...
const PUBLIC_SECTIONS = {
  ...
  agentExpenses: { fields: ["date","base","person","item","quantity","unit_price","amount","subtotal","notes"] },
