...
function openBaseFull(base, color, total, itemsJson) {
  ...
  var cats = ['土地流转','土地处理','种苗采购','地膜采购','农资','人工费用','其他'];
  var catColors = {'土地流转':'#8B4513','土地处理':'#D2691E','种苗采购':'#27ae60','地膜采购':'#9b59b6','农资':'#f39c12','人工费用':'#e74c3c','其他':'#95a5a6'};
  var catTotals = {};
  ...
  sortedI.forEach(function(r, idx){
    ...
    var note2 = (r.note||'').replace(/\//g,' · ').replace(/公司账户/g,'<span style="color:#27ae60">公司账户</span>').replace(/庞尚韬备用金/g,'<span style="color:#e67e22">庞备用金</span>').replace(/任海涛/g,'<span style="color:#3498db">任海涛</span>');
    detailRows += '<tr'+(idx%2===0?' style="background:#fafafa"':'')+'><td style="padding:3px 8px;font-size:0.7rem;font-weight:700;color:#fff;background:'+catBg+'88;border-radius:4px">'+(r.category||'')+'</td><td style="padding:5px 8px;font-size:0.72rem">'+(r.date||'')+'</td><td style="padding:5px 8px;font-size:0.72rem">'+(r.item||'')+'</td><td class="amount" style="padding:5px 8px;font-size:0.75rem;text-align:right">'+formatNum(r.amount)+'</td><td style="padding:5px 8px;font-size:0.65rem;color:#aaa;line-height:1.5;word-break:break-word;max-width:250px">'+note2+'</td><td style="padding:5px 8px;text-align:center">'+invHtml+'</td></tr>';
  ...
  body.innerHTML = '<div style="display:flex;gap:16px;align-items:stretch;margin-bottom:16px">' +
    '<div style="width:140px;height:140px;flex-shrink:0"><canvas id="baseFullChart"></canvas></div>' +
    '<div style="flex:1;min-width:0">' +
      '<table style="width:100%;border-collapse:collapse;font-size:0.72rem">' +
        ...
        cats.filter(function(c){return catTotals[c]>0;}).map(function(c){
          ...
          var barW = Math.max(pct*2, 4);
          return '<tr><td style="padding:3px 8px;white-space:nowrap;color:'+color+';font-weight:600">'+c+'</td><td style="padding:3px 8px;text-align:right;font-weight:600;font-size:0.75rem">'+formatNum(catTotals[c])+'</td><td style="padding:3px 8px;text-align:center;color:#888;font-size:0.65rem">'+pct+'%</td><td style="padding:3px 0;width:80px"><div style="height:10px;background:'+color+'22;border-radius:5px;overflow:hidden"><div style="height:100%;width:'+barW+'px;background:'+color+';border-radius:5px"></div></div></td></tr>';
        }).join('') +
        '<tr style="border-top:1px solid '+color+'44"><td style="padding:3px 8px;font-weight:700">合计</td><td style="padding:3px 8px;text-align:right;font-weight:700;font-size:0.85rem">'+formatNum(total)+'</td><td colspan="2"></td></tr>' +
        '</tbody></table>' +
    '</div></div>' +
    '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:10px">' +
    cats.filter(function(c){return catTotals[c]>0;}).map(function(cat){
      var bg = catColors[cat] || '#95a5a6';
      var icon = {'土地流转':'🏞️','土地处理':'🚜','种苗采购':'🌱','地膜采购':'🛍️','农资':'🧪','人工费用':'👷','其他':'📦'}[cat] || '📋';
      ...
      return '<div style="background:linear-gradient(135deg,'+bg+'11,'+bg+'22);border:1px solid '+bg+'44;border-radius:10px;cursor:pointer;text-align:center;padding:14px 8px;transition:all 0.25s" '+
        'onclick="openCatDetail(\''+cat+'\',\''+bg+'\','+catAmt+',\''+json+'\',\''+(base||'')+'\')" '+
        'onmouseover="this.style.transform=\'translateY(-3px)\';this.style.boxShadow=\'0 6px 20px '+bg+'44\'" '+
        'onmouseout="this.style.transform=\'\';this.style.boxShadow=\'\'">'+
        '<div style="font-size:1.8rem;margin-bottom:4px">'+icon+'</div>'+
        '<div style="font-size:0.7rem;font-weight:700;color:'+bg+'">'+cat+'</div>'+
        '<div style="font-size:0.85rem;font-weight:800;color:#1a1a1a;margin-top:4px">'+formatNum(catAmt)+'</div>'+
        '<div style="font-size:0.55rem;color:#888;margin-top:2px">'+pct+'% · '+catItems.length+'笔</div></div>';
    }).join('') + '</div>';
...
function openCatDetail(cat, color, total, itemsJson, baseName) {
  ...
  body.innerHTML = '<div style="margin-bottom:10px"><button onclick="if(_lastBaseContext){openBaseFull(_lastBaseContext.base,_lastBaseContext.color,_lastBaseContext.total,_lastBaseContext.items)}else{closeBaseFull()}" style="padding:8px 18px;border:2px solid '+color+';border-radius:8px;background:'+color+';color:#fff;cursor:pointer;font-size:14px;font-weight:700;box-shadow:0 2px 8px '+color+'44">← 返回上级</button></div>' +
...
function renderBaseExpense() {
  ...
  var categories = ['土地流转','土地处理','种苗采购','地膜采购','农资','人工费用','其他'];
  var catColors = {'土地流转':'#8B4513','土地处理':'#D2691E','种苗采购':'#27ae60','地膜采购':'#9b59b6','农资':'#f39c12','人工费用':'#e74c3c','其他':'#95a5a6'};
  ...
  baseNames.forEach(function(base, bi){
    ...
    categories.forEach(function(cat){
      ...
      var bg = catColors[cat] || '#95a5a6';
      var catLabel = cat;
      var catIcon = {'土地流转':'🏞️','土地处理':'🚜','种苗采购':'🌱','地膜采购':'🛍️','农资':'🧪','人工费用':'👷','其他':'📦'}[cat] || '📋';
...
function renderChart10b() {
  ...
  var catColors = {'土地流转':'#8B4513','土地处理':'#D2691E','种苗采购':'#27ae60','地膜采购':'#9b59b6','农资':'#f39c12','人工费用':'#e74c3c','其他':'#95a5a6'};
  var baseNames = ['金银花基地','党参基地','党参育苗基地'];
  var categories = ['土地流转','土地处理','种苗采购','地膜采购','农资','人工费用','其他'].filter(function(c){
