...
window.renderEditTable = function(section) {
  ...
  if (section === 'reimburse' || section === 'pettyCash') {
    ...
    persons.forEach(function(person) {
      ...
      dates.forEach(function(d) {
        ...
        cols.forEach(function(c) { 
          if(c.key==='date') ph += '<th style="width:80px">' + c.label + '</th>';
        ...
        items.forEach(function(row) {
          ...
          cols.forEach(function(c) {
            ...
            if (c.type === 'number') {
              ...
              ph += '<input type="date" value="' + ((val||'').split(' ')[0]||'') + '" data-row="' + realIdx + '" data-col="' + c.key + '" onchange="editCell(this)" style="width:80px;padding:2px 4px;border:1px solid #ccc">';
            } else if (c.key === 'docs') {
  ...
  cols.forEach(function(c) { 
    if(c.key==='date') html += '<th style="width:80px">' + c.label + '</th>';
