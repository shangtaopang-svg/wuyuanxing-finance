...
window.renderEditTable = function(section) {
  ...
  if (section === 'reimburse' || section === 'pettyCash') {
    ...
    persons.forEach(function(person) {
      ...
      dates.forEach(function(d) {
        ...
        cols.forEach(function(c) { 
          if(c.key==='date') ph += '<th style="width:65px">' + c.label + '</th>';
          else if(c.key==='amount') ph += '<th style="width:70px">' + c.label + '</th>';
          else if(c.key==='reason') ph += '<th>' + c.label + '</th>';
  ...
  cols.forEach(function(c) { 
    if(c.key==='date') html += '<th style="width:65px">' + c.label + '</th>';
    else if(c.key==='amount') html += '<th style="width:70px">' + c.label + '</th>';
    else if(c.key==='reason') html += '<th>' + c.label + '</th>';
